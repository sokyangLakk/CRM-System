...
export class CleaningTaskRepository {
  ...
  static async update(id: number, data: Partial<CleaningTask>): Promise<boolean> {
    const fields: string[] = [];
    const values: any[] = [];

    if (data.task_name) { fields.push('task_name = ?'); values.push(data.task_name); }
