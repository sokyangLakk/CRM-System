...
export class StudentRepository {
  static async create(student: Student): Promise<number> {
    const [result] = await pool.execute<ResultSetHeader>(
      'INSERT INTO students (user_id, name, student_number, class_id, gender, email, phone, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [student.user_id || null, student.name, student.student_number, student.class_id || null, student.gender || null, student.email || null, student.phone || null, student.status || 'active']
    );
